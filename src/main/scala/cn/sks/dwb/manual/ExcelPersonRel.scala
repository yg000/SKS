  ...
  def main(args: Array[String]): Unit = {
    ...
    val excel_person_reward=spark.sql(
      """
        ...
        |from dwb.wb_person_excel_reward_person_rel_one
        |""".stripMargin)
    //  团队与奖励
    ...
    val excel_team=spark.sql(
      """
        |select md5(team_name) as team_id,team_name  from dwd.wd_manual_excel_reward_team
        |""".stripMargin).dropDuplicates("team_name")


    //2---- 专项
    // 评审专家中的专项  -- -指南编制专家中的专项  -- porject 中的专项
    val excel_special_project=spark.sql("select md5(special_name) as id, special_name,special_type,'国家级' as special_level,publish_department from ods.o_manual_excel_review_expert")
      .union(
    spark.sql("select md5(special_name) as id,special_name,special_type,'国家级' as special_level,publish_department from ods.o_manual_excel_guide_compilation_expert"))
      .union(
    ...
    val excel_person_special_project = spark.sql(
      """
        ...
        |from dwb.wb_person_excel_review_expert_person_rel_one
        |""".stripMargin)
    //3---- 人才
    ...
    val excel_person_outstanding= spark.sql(
      """
        ...
        |from dwb.wb_person_excel_outstanding_person_rel_one
        |""".stripMargin)
    ...
    val excel_person_guide = spark.sql(
      """
        ...
        |from dwb.wb_person_excel_guide_compilation_expert_person_rel_one
        |""".stripMargin)



    println(excel_team                  .count())
    ...
    excel_team.createOrReplaceTempView("excel_team")
    ...
    spark.sql("insert into table dwb.wb_excel_team                     select * from excel_team         ")
    spark.sql("insert into table dwb.wb_excel_reward                   select * from excel_reward                ")
    spark.sql("insert into table dwb.wb_excel_team_reward              select * from excel_team_reward         ")
    spark.sql("insert into table dwb.wb_excel_person_reward            select * from excel_person_reward         ")
    spark.sql("insert into table dwb.wb_excel_special_project          select * from excel_special_project       ")
    spark.sql("insert into table dwb.wb_excel_person_special_project   select * from excel_person_special_project")
    spark.sql("insert into table dwb.wb_excel_outstanding              select * from excel_outstanding           ")
    spark.sql("insert into table dwb.wb_excel_person_outstanding       select * from excel_person_outstanding    ")
    spark.sql("insert into table dwb.wb_excel_guide                    select * from excel_guide                 ")
    spark.sql("insert into table dwb.wb_excel_guide_special_project    select * from excel_guide_special_project ")
    spark.sql("insert into table dwb.wb_excel_person_guide             select * from excel_person_guide          ")
