...
object FusionReviewExpert {

  def main(args: Array[String]): Unit = {
    val spark = SparkSession.builder()
      .master("local[12]")
      .appName("FusionReviewExpert")
    ...
    val manual_excel = spark.sql(
      """
        ...
        | from dwd.wd_manual_excel_review_expert
        |""".stripMargin)
    manual_excel.createOrReplaceTempView("manual_excel")
    // 两部分人中的交集（人员对应关系）
    val rel_person= spark.sql(
      """
        | select a.person_id,a.id,special_name,special_type,review_type,review_group,review_rounds,review_year from (
        |    select * from (
    ...
    //一对一 写入 hive库
    spark.sql("insert into table dwb.wb_person_manual_excel_review_expert_person_rel_one   select * from rel_person_one")
    // 一对多 写入hive库 （为后续人工做准备）
    spark.sql("insert into table dwb.wb_person_manual_excel_review_expert_person_rel_more  select * from rel_person_more")
    ...
    spark.sql("insert into table dwb.wb_person_manual_excel_review_expert_person_not_fusion   select * from person_not_exists")
