...
object PersonFusionAcademician {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val flag = "person_nsfc_sts_academician_rel"
...
//    val distinct_relation = PersonUtil.getDeliverRelation(spark,person_from_distinct_rule1_rel,person_from_distinct_rule2_rel).union(person_from_distinct_rule2_rel)
    ...
    spark.sql(
      """
        |select count(*),count(distinct person_id) from  mid where clean_title is not null
        |""".stripMargin)//.show()
    val person_from_with_title  = PersonUtil.getAddTitle(spark,person_from,"dwd.wd_product_person_ext_csai")

    val person_fusion_1 = PersonUtil.getComparisonTable(spark,person_to,person_from,"zh_name","clean_org","birth_year","zh_name+org_name+birthday")
    ...
    person_to.unionAll(person_from).createOrReplaceTempView("person_nsfc_sts_academician")

    person_fusion_relation.repartition(2).createOrReplaceTempView("person_rel")
    spark.sql(
      s"""
