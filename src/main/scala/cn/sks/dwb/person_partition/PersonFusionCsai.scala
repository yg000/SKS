  ...
  def main(args: Array[String]): Unit = {
    ...
    spark.sql(
      """
        ...
        |,zh_title from dwd.wd_product_person_ext_csai a left join dwb.wb_person_rel_partition b on a.person_id = b.person_id_from
        |""".stripMargin).createOrReplaceTempView("wd_product_person_ext_csai")

    spark.sql(
      """
        |select
        |ifnull(person_id_to,person_id) as person_id
        |,zh_title from dwd.wd_product_person_ext_nsfc a left join dwb.wb_person_rel_partition b on a.person_id = b.person_id_from
        |""".stripMargin).createOrReplaceTempView("wd_product_person_ext_nsfc")
