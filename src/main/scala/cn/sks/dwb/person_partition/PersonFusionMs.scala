...
object PersonFusionMs {
  def main(args: Array[String]): Unit = {
    ...
    var person_from_distinct_rule1_rel = PersonUtil.getDistinctRelation(spark,person_from_distinct_with_title.select("clean_title","clean_en_name_normal","person_id"),person_from_distinct_rule1.select("clean_title","clean_en_name_normal","person_id"),"clean_title","clean_en_name_normal","","zh_name+title")
    ...
    var person_from_distinct_rule2_rel = PersonUtil.getDistinctRelation(spark,person_from_distinct_rule1.select("clean_title","clean_en_name_inverted","person_id"),person_from_distinct_rule2.select("clean_title","clean_en_name_inverted","person_id"),"clean_title","clean_en_name_inverted","","zh_name+title")
    ...
    PersonUtil.getDeliverRelation (spark,distinct_relation,person_fusion_relation).unionAll(person_fusion_relation.select("person_id_from","person_id_to")).dropDuplicates("person_id_from","person_id_to").cache()
