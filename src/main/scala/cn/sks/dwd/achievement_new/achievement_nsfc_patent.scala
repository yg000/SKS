...
object achievement_nsfc_patent {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val fieldNames01 = spark.read.table("ods.o_nsfc_npd_patent").schema.fieldNames
    val fieldNames02 = spark.read.table("ods.o_nsfc_project_patent").schema.fieldNames
    val fieldNames03 = spark.read.table("ods.o_nsfc_product_patent").schema.fieldNames
    val fieldNames = fieldNames01.intersect(fieldNames02).intersect(fieldNames03)
    println(fieldNames.toBuffer)
    spark.read.table("ods.o_nsfc_npd_patent").select(fieldNames(0),fieldNames.drop(0):_*).unionAll(
    spark.read.table("ods.o_nsfc_product_patent").select(fieldNames(0),fieldNames.drop(0):_*)).unionAll(
    spark.read.table("ods.o_nsfc_project_patent").select(fieldNames(0),fieldNames.drop(0):_*)).createOrReplaceTempView("nsfc_patent")

    spark.sql(
      """
        |insert overwrite table dwd.wd_achievement_nsfc partition(achievement_type = 'patent')
        |select
        |achievement_id
