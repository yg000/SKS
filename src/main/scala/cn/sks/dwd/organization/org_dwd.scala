...
object org_dwd {
  ...
  spark.sqlContext.udf.register("TransformEnglish", (str: String) => {
    val APP_ID: String ="20200617000497648"
    val SECURITY_KEY: String ="AWuX5fymN1X61Bfxkxi4"
  ...
  def main(args: Array[String]): Unit = {
   ...
   spark.sql("""
               |select  pinyin,split(merger_name,',')[1] as province,short_name,name,zip_code from (select * ,row_number()over(partition by pinyin order by level_type,id) as tid from dictionary_china_region where level_type in('2','3'))a where tid = 1
               |""".stripMargin).createOrReplaceTempView("dictionary_china_region")


    //spark.sql("select TransformChinese(city) as city from   ods.o_manual_organization_standards_institutions limit 2").show(100)


    spark.sql("""
    |select  * from (select * ,row_number()over(partition by clean_fusion(chinese_name_of_organization) order by id desc) as tid from ods.o_manual_organization_standards_institutions )a where tid = 1
    |""".stripMargin).createOrReplaceTempView("manual_organization_standards_institutions")

    //spark.sql("select count(*) from   manual_organization_standards_institutions").show(10)

    spark.sql( """
                 |select * from (select *,row_number()over(partition by clean_fusion(inst_name) order by id desc) as tid from ods.o_manual_organization_zky_inst)a where tid =1
                 |""".stripMargin).createOrReplaceTempView("manual_organization_zky_inst")

    spark.sql("""
                |select  * from (select * ,row_number()over(partition by clean_fusion(org_name) order by registration_date ) as tid from ods.o_manual_organization_sts )a where tid = 1
                |""".stripMargin).createOrReplaceTempView("manual_organization_sts")

    spark.sql("""
                |select  * from (select * ,row_number()over(partition by clean_fusion(org_name) order by established ) as tid from ods.o_manual_organization_grid )a where tid = 1
                |""".stripMargin).createOrReplaceTempView("manual_organization_grid")
