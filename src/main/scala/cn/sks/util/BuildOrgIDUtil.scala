  ...
  def buildOrganizationID(spark:SparkSession,oldDf:DataFrame,org_name:String,source:String):DataFrame={
    ...
    spark.sql(
      s"""
         |select a.*,c.org_id
         | from
         |(select ifnull(standard_name,a.$org_name )  as new_org_name,a.* from oldtb a left join wb_organization_comparison_table b on non_standard_name = a.$org_name) a
         |left join  wb_organization c on clean_fusion(a.new_org_name) = clean_fusion(c.org_name)
         |""".stripMargin).drop("new_org_name")
