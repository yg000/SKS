...
//                |""".stripMargin).createOrReplaceTempView("wb_organization_comparison_table")
    ...
    spark.sql(
      s"""
         ...
         |from
         |(select ifnull(standard_name,a.$org_name )  as new_org_name,a.* from oldtb a left join dwb.wb_organization_comparison_table b on non_standard_name = a.$org_name) a
