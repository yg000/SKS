...
//                |""".stripMargin).createOrReplaceTempView("wb_organization_comparison_table")
  ...
  def buildOrganizationExpID(spark:SparkSession,oldDf:DataFrame,org_name:String,source:String):DataFrame={
    ...
    spark.sql(
      s"""
         |select a.org_name as org_name
         | from
         |(select ifnull(standard_name,a.$org_name )  as org_name from   oldtb  a left join wb_organization_comparison_table b on non_standard_name = a.$org_name ) a
         |left join wb_organization c on clean_fusion(a.org_name) = clean_fusion(c.org_name)  where c.org_name is null
         |""".stripMargin).createOrReplaceTempView("org_product_lack")
    ...
    spark.sql(
      s"""
         ...
         |null as zipcode,
         |null as source
         | from org_product_lack where org_name is not null and length(org_name)>2
         |""".stripMargin)
    ...
    spark.sql(
      s"""
         |select a.*,c.org_id,c.org_name as new_org_name
         |from
         |(select ifnull(standard_name,a.$org_name )  as tmp_org_name,a.* from oldtb a left join wb_organization_comparison_table b on non_standard_name = a.$org_name) a
         | join  wb_organization c on clean_fusion(a.tmp_org_name) = clean_fusion(c.org_name)
         |""".stripMargin).drop("tmp_org_name")
