...
object PersonUtil {
  ...
  def getDistinctRelationSecond(spark:SparkSession,df:DataFrame,col_1:String,col_2:String,col_3:String):DataFrame = {
    ...
    if ("" == col_3 ){
      spark.sql(
        s"""
           ...
           |from mid_tb a join mid_tb b on a.$col_1 = b.$col_1 and  a.$col_2 = b.$col_2  where a.person_id != b.person_id
           |""".stripMargin).dropDuplicates("person_id_from")
    }else {
      spark.sql(
        s"""
           |select a.person_id as person_id_from,b.person_id as person_id_to
           |from mid_tb a join mid_tb b on a.$col_1 = b.$col_1 and  a.$col_2 = b.$col_2 and a.$col_3 = b.$col_3 where a.person_id != b.person_id
           |""".stripMargin).dropDuplicates("person_id_from")
    }
  ...
  def getDistinctRelation(spark:SparkSession,originDf:DataFrame,distinctDf:DataFrame,col_1:String,col_2:String,col_3:String):DataFrame = {
    originDf.createOrReplaceTempView("origin_tb")
    distinctDf.dropDuplicates("person_id").createOrReplaceTempView("distinct_tb")

    if ("" == col_3 ){
      spark.sql(
        s"""
           |select * from (
           |    select a.person_id as `person_id_from`,b.person_id as `person_id_to` from origin_tb a join distinct_tb b
           |    on a.$col_1 = b.$col_1 and  a.$col_2 = b.$col_2
           |)a where `person_id_from` <> person_id_to
          """.stripMargin)
    }else {
      spark.sql(
        s"""
